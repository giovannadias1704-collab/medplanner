...
// Todas as rotas que usam PageLayout (têm sidebar própria)
// Navigation fica completamente oculto nessas páginas
const SIDEBAR_PAGES = [
  '/dashboard',
  '/analytics',
  '/estrategia',
...
export default function Navigation() {
  ...
  // Paleta (usada apenas nas rotas públicas como /landing, /auth, /onboarding)
  const S = {
    bg:      '#FFFCF7',
    border:  '#E8DFD3',
    text:    '#3E3A36',
    textSec: '#6B5E53',
    badge:   '#EFE6D8',
    active:  { text: '#8FA889', bg: '#EAF1E8' },
    logo:    { icon: '#A8BFA3', text: '#5C7A57' },
  ...
  return (
    <>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 20px', width: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 60 }}>
            <Link to="/dashboard" style={{ display: 'flex', alignItems: 'center', gap: 9, textDecoration: 'none', flexShrink: 0 }}>
              ...
              <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 16, color: S.logo.text, letterSpacing: '-0.3px' }}>
                MedPlanner
              </span>
            </Link>
                ...
                return (
                  <Link
                    ...
                    style={{
                      ...
                      color: isActive ? (isAdminLink ? S.admin.text : S.active.text) : isAdminLink ? S.admin.text : S.textSec,
                    }}
                    onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = S.badge; }}
      ...
      <nav className="md:hidden" style={{
        ...
        boxShadow: '0 -2px 12px rgba(120,100,80,0.08)', zIndex: 9999,
      }}>
          ...
          {navItems.slice(0, 5).map(item => {
            ...
            return (
              <Link key={item.path} to={item.path} style={{
                ...
                <span style={{ fontFamily: "'Poppins',sans-serif", fontSize: 10, fontWeight: isActive ? 600 : 400 }}>{item.label}</span>
              </Link>
            );
          })}
        </div>
