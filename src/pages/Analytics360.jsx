...
import PageLayout from '../components/PageLayout'; // ✅ ADICIONADO
...
export default function Analytics() {
  ...
  const scores = useMemo(() => {
    ...
    let prodScore = 0;
    let prodCount = 0;
    if (tasks.length > 0) { prodScore += (tasks.filter(t => t.completed).length / tasks.length) * 100; prodCount++; }
    if (evts.length > 0) { prodScore += (evts.filter(e => e.completed).length / evts.length) * 100; prodCount++; }
    if (study.length > 0) {
  ...
  // ✅ CONTEÚDO DA PÁGINA (sem o wrapper de layout antigo)
  const pageContent = (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;700;800&family=DM+Sans:ital,wght@0,300;0,400;0,500;1,300&display=swap');
        .v360-root { background: #050d1a; padding: 8px 0 60px; font-family: 'DM Sans', sans-serif; color: #e2e8f0; border-radius: 16px; }
      ...
      <div className="v360-root" style={{ padding: '20px 24px 60px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div className="v360-tabs">
            {[
              ...
              <button key={t.key} className={`v360-tab${activeTab === t.key ? ' active' : ''}`} onClick={() => handleTabChange(t.key)}>
                {t.label}
                ...
                <>
                  ...
                  <div className="burnout-card">
                    ...
                    <div className="burnout-bar-wrap">
                      <div className="burnout-bar-fill" style={{ width: `${burnoutRisk}%`, background: burnoutRisk > 70 ? '#ef4444' : burnoutRisk > 40 ? '#f59e0b' : '#22d3ee' }} />
                    </div>
                      ...
                      <div className="alerts-section">
                        {alerts.map((a, i) => (
                          <div key={i} className={`alert-item alert-${a.type}`}>
                            <span className="alert-icon">{a.type === 'danger' ? '🔴' : a.type === 'warning' ? '🟡' : '🔵'}</span>
                            <span>{a.msg}</span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="sparkline-grid">
                    {[
                      { name: 'Sono', values: timeSeries.sleep, color: '#22d3ee' },
                      { name: 'Humor', values: timeSeries.mood, color: '#a78bfa' },
                      { name: 'Exercício', values: timeSeries.exercise, color: '#fbbf24' },
                      { name: 'Hidratação', values: timeSeries.water, color: '#34d399' },
                    ].map(s => {
                      ...
                      return (
                        <div key={s.name} className="spark-card">
                          ...
                          <div className="spark-value" style={{ color: s.color }}>{avg}<span style={{ fontSize: '0.9rem', color: '#475569' }}>/100</span></div>
                  ...
                  {correlations.length > 0 && (
                    <div className="section-box">
                      ...
                      <table className="corr-table">
                        <thead><tr><th>Relação</th><th>Correlação</th><th>Força</th><th>Impacto</th></tr></thead>
                        <tbody>
                            ...
                            return (
                              <tr key={i}>
                                <td>
                                  ...
                                  <div className="corr-insight">{c.r > 0 ? 'Correlação positiva' : 'Correlação inversa'} — r = {c.r}</div>
                                </td>
                                <td><div className="corr-bar-wrap"><div className="corr-bar-fill" style={{ width: `${pct}%`, background: color }} /></div></td>
                                <td style={{ color }}>{strength}</td>
                                <td><span className={`impact-badge impact-${strength.toLowerCase()}`}>{c.impact}</span></td>
                              </tr>
                            );
                          })}
                        </tbody>
                      ...
                      {correlations.slice(0, 5).map((c, i) => {
                        ...
                        return (
                          <div key={i} className="inf-item">
                            ...
                            <div className="inf-bar-outer"><div className="inf-bar-inner" style={{ width: `${abs*100}%`, background: color }} /></div>
                            <span className="inf-r" style={{ color }}>{c.r > 0 ? '+' : ''}{c.r}</span>
                          </div>
                        );
                      })}
                      {correlations.length === 0 && <p style={{ color: '#475569', fontSize: '0.83rem' }}>Registre mais dados de bem-estar para detectar correlações.</p>}
                    </div>
                  </div>
              ...
              <div>
                ...
                {todayWell ? (
                  ...
                  <div className="empty-state">
                    ...
                    <div className="empty-actions"><a href="/wellness" className="btn-primary">Registrar Bem-estar</a></div>
                  </div>
                )}
              </div>
            );
          ...
          {activeTab === 'comparativo' && (
            !hasFeature('advancedAnalytics') ? (
              <PremiumBlock feature="advancedAnalytics" requiredPlan="student" message="Analytics comparativo com filtros de período, score de produtividade e insights avançados estão disponíveis nos planos Estudante e superiores." />
            ) : (
              <div>
                <div className="period-filter">
  ...
  // ✅ RETORNO FINAL — envolve com PageLayout
  return (
